...
def render_avatar_sidebar(p="#00ffff", g="0,255,255", br="0,200,255"):
    st.markdown(f"""
    <style>
    .rei-avatar-wrap {display:flex;flex-direction:column;align-items:center;padding:10px 0 6px 0;}
    .rei-holo-ring {position:relative;width:170px;height:170px;display:flex;align-items:center;justify-content:center;cursor:pointer;}
    .rei-holo-ring::before {content:"";position:absolute;inset:-4px;border-radius:50%;background:conic-gradient(rgba({g},0.9) 0deg,rgba({g},0.1) 90deg,rgba({g},0.8) 180deg,rgba({g},0.2) 270deg,rgba({g},0.9) 360deg);animation:ringRotate 3s linear infinite;z-index:0;}
    .rei-holo-ring::after {content:"";position:absolute;inset:2px;border-radius:50%;background:#000010;z-index:1;}
    @keyframes ringRotate {from{transform:rotate(0deg);}to{transform:rotate(360deg);}}
    .rei-svg-container {position:relative;z-index:2;width:154px;height:154px;border-radius:50%;overflow:hidden;box-shadow:0 0 20px rgba({g},0.6),0 0 50px rgba({g},0.25);transition:box-shadow 0.2s ease;}
    .rei-svg-container.speaking {box-shadow:0 0 40px rgba({g},1),0 0 90px rgba({g},0.6)!important;animation:speakPulse 0.3s ease-in-out infinite alternate;}
    @keyframes speakPulse {from{box-shadow:0 0 35px rgba({g},0.9);}to{box-shadow:0 0 55px rgba({g},1);}}
    .rei-scanlines {position:absolute;inset:0;border-radius:50%;background:repeating-linear-gradient(0deg,transparent,transparent 4px,rgba({g},0.03) 4px,rgba({g},0.03) 6px);pointer-events:none;z-index:3;animation:scanMove 2s linear infinite;}
    @keyframes scanMove {from{background-position:0 0;}to{background-position:0 60px;}}
    .rei-voice-bars {display:none;justify-content:center;gap:3px;margin:8px 0 2px 0;height:22px;align-items:flex-end;}
    .rei-voice-bars.active {display:flex;}
    .rei-vbar {width:4px;background:{p};border-radius:2px;box-shadow:0 0 6px rgba({g},0.8);}
    ...
    @keyframes vbarAnim {0%,100%{transform:scaleY(0.35);opacity:0.6;}50%{transform:scaleY(1);opacity:1;}}
    </style>
    <div class="rei-avatar-wrap">
      <div class="rei-holo-ring" onclick="window.reihanaStop()">
        <div class="rei-svg-container" id="reiAvatarBox">
          <svg viewBox="0 0 154 154" xmlns="http://www.w3.org/2000/svg" width="154" height="154">
            <defs>
              ...
              <radialGradient id="eyeGl" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="rgba({g},0.5)"/><stop offset="100%" stop-color="rgba({g},0)"/></radialGradient>
              ...
              <filter id="sfGlow"><feGaussianBlur stdDeviation="1.5" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
            ...
            <g clip-path="url(#cc)">
              ...
              <path d="M62,118 L77,128 L92,118 L88,115 L77,124 L66,115 Z" fill="rgba({g},0.3)" filter="url(#sfGlow)"/>
              <line x1="77" y1="128" x2="77" y2="154" stroke="rgba({g},0.5)" stroke-width="1.5" opacity="0.7"/>
              ...
              <ellipse cx="62" cy="80" rx="8" ry="8.5" fill="url(#eyeGl)" opacity="0.6"/>
              ...
              <ellipse cx="92" cy="80" rx="8" ry="8.5" fill="url(#eyeGl)" opacity="0.6"/>
              ...
              <path id="lipTop" d="M67,106 C70,103 74,102 77,102 C80,102 84,103 87,106" stroke="#c06070" stroke-width="1.8" fill="none" stroke-linecap="round"/>
              <path id="lipBot" d="M67,106 C70,109 74,110 77,110 C80,110 84,109 87,106" fill="#e08090" stroke="#c06070" stroke-width="0.8" opacity="0.9"/>
              ...
              <ellipse id="eyeGlL" cx="62" cy="80" rx="0" ry="0" fill="rgba({g},0.4)" opacity="0"/>
              <ellipse id="eyeGlR" cx="92" cy="80" rx="0" ry="0" fill="rgba({g},0.4)" opacity="0"/>
              <g transform="translate(100,38)" filter="url(#sfGlow)">
      ...
      <div class="rei-voice-bars" id="reiVoiceBars">
        <div class="rei-vbar"></div><div class="rei-vbar"></div><div class="rei-vbar"></div>
        <div class="rei-vbar"></div><div class="rei-vbar"></div>
      </div>
    </div>
...
(function(){
  ...
  var _spk=false,_bt=null,_mt=null;
  function blink(){
    var eL=g("eyelidL"),eR=g("eyelidR"),pL=g("pupilL"),pR=g("pupilR");
    if(!eL||!eR)return;
  ...
  function sb(){_bt=setTimeout(function(){blink();sb();},2500+Math.random()*3000);}
  function mouth(lv){
    var lT=g("lipTop"),lB=g("lipBot"),mi=g("mouthIn"),mt=g("mouthTth");
    if(!lT||!lB)return;
    var oh=lv*7;
    if(lv<0.05){
  ...
  function startSpk(){
    _spk=true;
    var b=g("reiAvatarBox"),bars=g("reiVoiceBars");
    if(b)b.classList.add("speaking");if(bars)bars.classList.add("active");
    glow(true);
  }
  function stopSpk(){
    _spk=false;
    var b=g("reiAvatarBox"),bars=g("reiVoiceBars");
    if(b)b.classList.remove("speaking");if(bars)bars.classList.remove("active");
    glow(false);mouth(0);
  }
  var _oS=window.reihanaSpeak,_oSt=window.reihanaStop;
  window.reihanaSpeak=function(t){startSpk();if(_oS)_oS(t);setTimeout(function(){if(_spk)stopSpk();},Math.max(1500,t.length*55)+500);};
  window.reihanaStop=function(){stopSpk();if(_oSt)_oSt();};
  if(window.speechSynthesis){
    var ns=window.speechSynthesis.speak.bind(window.speechSynthesis);
    window.speechSynthesis.speak=function(u){u.addEventListener("start",startSpk);u.addEventListener("end",stopSpk);u.addEventListener("error",stopSpk);ns(u);};
  }
