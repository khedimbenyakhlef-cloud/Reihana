...
.hologram-container { position: relative; width: 100%; text-align: center; padding: 20px 0; }
.hologram-avatar {
    ...
    box-shadow: 0 0 30px rgba(0,255,255,0.4), 0 0 60px rgba(0,150,255,0.2), inset 0 0 30px rgba(0,255,255,0.1);
    display: flex; align-items: center; justify-content: center; font-size: 80px;
    animation: holoPulse 3s ease-in-out infinite, holoRotate 8s linear infinite;
    position: relative; flex-direction: column; cursor: pointer;
}
.holo-face { position: relative; display: flex; flex-direction: column; align-items: center; gap: 4px; }
.holo-eyes { display: flex; gap: 18px; margin-top: 10px; }
.holo-eye { width: 10px; height: 10px; background: #00ffff; border-radius: 50%; box-shadow: 0 0 6px #00ffff; animation: eyeBlink 4s ease-in-out infinite; }
@keyframes eyeBlink { 0%, 90%, 100% { transform: scaleY(1); } 95% { transform: scaleY(0.1); } }
.holo-mouth {
    width: 30px; height: 10px; background: transparent; border: 2px solid #00ffff;
    border-top: none; border-radius: 0 0 20px 20px; margin-top: 8px;
    transition: all 0.1s ease; box-shadow: 0 0 8px rgba(0,255,255,0.6);
}
.holo-mouth.talking { animation: mouthTalk 0.25s ease-in-out infinite alternate; }
@keyframes mouthTalk {
...
.hologram-avatar::after {
    ...
    animation: scanLines 2s linear infinite; pointer-events: none;
}
@keyframes holoPulse {
...
.speaking-indicator { display: flex; justify-content: center; align-items: flex-end; gap: 3px; height: 20px; margin-top: 6px; opacity: 0; transition: opacity 0.3s; }
.speaking-indicator.active { opacity: 1; }
.speaking-indicator span { width: 3px; background: #00ffff; border-radius: 2px; animation: wave 0.8s ease-in-out infinite; }
...
.reihana-title {
    font-family: 'Orbitron', monospace !important; font-size: 2.5rem !important; font-weight: 900 !important;
    background: linear-gradient(135deg, #00ffff, #0088ff, #aa00ff, #00ffff); background-size: 300% 300%;
    -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: gradientShift 4s ease infinite;
    text-align: center; letter-spacing: 8px; margin: 10px 0;
}
@keyframes gradientShift { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
.reihana-subtitle { font-family: 'Rajdhani', sans-serif; color: rgba(0,255,255,0.6); text-align: center; font-size: 0.85rem; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 5px; }
.msg-reihana {
    background: linear-gradient(135deg, rgba(0,30,60,0.9), rgba(0,20,50,0.8)); border: 1px solid rgba(0,200,255,0.3);
    border-left: 3px solid #00ccff; border-radius: 0 15px 15px 15px; padding: 15px 18px; margin: 10px 0 4px 0;
    font-family: 'Rajdhani', sans-serif; font-size: 1rem; color: #c8f0ff; box-shadow: 0 4px 20px rgba(0,150,255,0.15); position: relative;
}
.msg-reihana::before { content: '🌸 REIHANA'; font-family: 'Orbitron', monospace; font-size: 0.65rem; color: #00ccff; display: block; margin-bottom: 8px; letter-spacing: 2px; }
.msg-user {
    background: linear-gradient(135deg, rgba(20,0,50,0.8), rgba(30,0,60,0.7)); border: 1px solid rgba(150,0,255,0.3);
    border-right: 3px solid #8800ff; border-radius: 15px 0 15px 15px; padding: 12px 16px; margin: 10px 0;
    font-family: 'Rajdhani', sans-serif; color: #e0d0ff; text-align: right;
}
.msg-user::before { content: 'VOUS 👤'; font-family: 'Orbitron', monospace; font-size: 0.65rem; color: #aa88ff; display: block; margin-bottom: 6px; letter-spacing: 2px; }
.action-bar { display: flex; gap: 8px; margin: 0 0 14px 4px; flex-wrap: wrap; }
.action-btn {
    background: rgba(0,200,255,0.07); border: 1px solid rgba(0,200,255,0.2); color: rgba(0,200,255,0.7);
    font-family: 'Rajdhani', sans-serif; font-size: 0.78rem; padding: 4px 10px; border-radius: 20px; cursor: pointer;
    transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 4px; letter-spacing: 1px; text-decoration: none;
}
.action-btn:hover { background: rgba(0,200,255,0.15); border-color: rgba(0,200,255,0.5); color: #00ccff; box-shadow: 0 0 10px rgba(0,200,255,0.2); }
.action-btn.liked { background: rgba(255,80,120,0.15); border-color: rgba(255,80,120,0.4); color: #ff5078; }
.deep-think-badge {
    background: linear-gradient(135deg, rgba(120,0,255,0.15), rgba(80,0,200,0.1)); border: 1px solid rgba(150,0,255,0.3);
    border-radius: 6px; padding: 4px 10px; font-family: 'Orbitron', monospace; font-size: 0.62rem; color: #aa88ff;
    letter-spacing: 1px; display: inline-block; margin-bottom: 8px;
}
.web-search-badge {
    background: linear-gradient(135deg, rgba(0,180,100,0.15), rgba(0,120,60,0.1)); border: 1px solid rgba(0,200,120,0.3);
    border-radius: 6px; padding: 4px 10px; font-family: 'Orbitron', monospace; font-size: 0.62rem; color: #00cc88;
    letter-spacing: 1px; display: inline-block; margin-bottom: 8px;
}
.stat-badge { background: rgba(0,255,255,0.08); border: 1px solid rgba(0,255,255,0.2); border-radius: 8px; padding: 8px 12px; font-family: 'Orbitron', monospace; font-size: 0.7rem; color: #00ccff; text-align: center; margin: 4px 0; }
.stTextArea textarea, .stTextInput input { background: rgba(0,20,50,0.8) !important; border: 1px solid rgba(0,200,255,0.3) !important; color: #c8f0ff !important; font-family: 'Rajdhani', sans-serif !important; border-radius: 10px !important; }
.stButton > button {
    background: linear-gradient(135deg, #001440, #002060) !important; border: 1px solid rgba(0,200,255,0.4) !important;
    color: #00ccff !important; font-family: 'Orbitron', monospace !important; font-size: 0.75rem !important;
    letter-spacing: 2px !important; border-radius: 8px !important; transition: all 0.3s ease !important;
}
.stButton > button:hover { background: linear-gradient(135deg, #002060, #003080) !important; box-shadow: 0 0 20px rgba(0,200,255,0.4) !important; transform: translateY(-2px) !important; }
...
.holo-line { height: 1px; background: linear-gradient(90deg, transparent, #00ccff, #8800ff, transparent); margin: 15px 0; animation: hololine 3s ease-in-out infinite; }
@keyframes hololine { 0%, 100% { opacity: 0.4; } 50% { opacity: 1; } }
.status-online { display: inline-block; width: 8px; height: 8px; background: #00ff88; border-radius: 50%; animation: blink 2s ease-in-out infinite; margin-right: 6px; }
@keyframes blink { 0%, 100% { opacity: 1; box-shadow: 0 0 6px #00ff88; } 50% { opacity: 0.4; box-shadow: none; } }
::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: rgba(0,0,20,0.5); } ::-webkit-scrollbar-thumb { background: rgba(0,200,255,0.4); border-radius: 2px; }
.stFileUploader { background: rgba(0,10,30,0.6) !important; border: 1px dashed rgba(0,200,255,0.3) !important; border-radius: 10px !important; }
...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        with st.spinner("🔍 Analyse en cours..."):
            ...
            st.session_state.fichiers_contexte.append({"nom": uploaded_file.name, "contenu": file_context})
...
if send_btn and user_input and user_input.strip():
    ...
    groq_messages = [{"role": msg["role"], "content": msg["content"]} for msg in st.session_state.messages[-10:]]
